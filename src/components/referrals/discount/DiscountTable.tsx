...
// src/components/referrals/DiscountTable.tsx
import { useState } from 'react';
...
import EditPopUpModal from './EditPopUpModal';
import LoadingSpinner from '../../ui/LoadingSpinner';
import { useReferralResponse } from '../../../hooks/sync/projects/useReferralData';
...
const DiscountTable = () => {
  ...
  // Fetch referrals data using the response hook
  const { data: referralResponse, isLoading, isError } = useReferralResponse();
  ...
  const addMutation = useMutation({
    mutationFn: (newReferral: Omit<Referral, 'id'>) => 
      referralService.addReferral(newReferral),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['referrals-response'] });
      toast({
        title: 'Referral added successfully',
        status: 'success',
        duration: 3000,
  ...
  if (referrals.length === 0) return (
    <div className="text-center py-10">
      ...
      <button 
        ...
        onClick={() => queryClient.invalidateQueries({ queryKey: ['referrals-response'] })}
      >
        Reload Data
      </button>
    </div>
